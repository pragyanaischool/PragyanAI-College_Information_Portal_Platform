...
def execute_college_sql(query: str) -> str:
    ...
    try:
        # Robustly parse SQLite URI path
        db_url = settings.DATABASE_URL
        if db_url.startswith("sqlite:///"):
            db_path = db_url.replace("sqlite:///", "")
            # Ensure path exists or resolve relative to base
            if not Path(db_path).exists():
                # Try fallback standard location
                fallback_path = Path(__file__).resolve().parent.parent.parent / "pragyanai_college_hub.db"
                if fallback_path.exists():
                    db_path = str(fallback_path)
            conn = sqlite3.connect(db_path)
        else:
            # Generic SQLAlchemy connection fallback
...
def query_vector_store_tool(
    ...
    try:
        ...
        if not results:
            return "No matching regulatory or brochure records found in vector store."
        ...
        return f"Vector Retrieval Fallback: Unable to query vector search index ({e}). Utilizing base model context."
...
def calculate_roi_tool(
    ...
    """
    try:
        total_investment = float(total_4yr_fee_lakhs) + (float(living_cost_per_year_lakhs) * 4.0)
        monthly_take_home_approx = (float(median_ctc_lpa) * 100000.0 * 0.82) / 12.0
        ...
        result = {
            ...
            "3_year_net_career_roi": round((float(median_ctc_lpa) * 3.5) - total_investment, 2),
        }
...
def web_search_fallback(query_text: str) -> str:
    ...
    try:
        ...
        return results if results else "No live web search results found."
    except Exception as e:
